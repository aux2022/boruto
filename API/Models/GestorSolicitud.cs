    ...
    public class GestorSolicitud
    {
       /*e*/ public List<solicitud> GetSolicituds()
        ...
        public List<solicitud> GetSolicitudsById(int id2)
        {
            ...
            using (SqlConnection conn = new SqlConnection(strConn))
            {
                ...
                SqlCommand cmd = new SqlCommand("select s.idSolicitud, s.nombreSolicitante, s.correo, s.fechaSolicitud, s.horaSolicitud, s.area, s.maquina, s.dispositivo, s.descripcionProblema, d.nomina, m.nombre, d.fechaInicio, d.horaInicio, d.diagnostico, d.tipoFalla, d.emailSent, d.nomina2, m2.nombre as nombre2, d.asignacion, r.tareasEjecutadas, r.generoParo, r.paroCorrectivo, r.paroOperativo, r.paroRefacccion, r.tiempoTotal, r.grasaUtilizada, r.refaMateHerra, c.fechaFinal, c.horaFinal, c.trabajoSanitizado, c.estatusActividad, c.firmaSolicitante, c.emailSent2 from SolicitudPreventiva as S inner join Diagnostico as D on S.idSolicitud=D.idSolicitud inner join Reparacion as R on D.idSolicitud= R.idSolicitud inner join CierreSolicitud as C on R.idSolicitud= C.idSolicitud inner join Mecanicos M on D.nomina=M.nomina inner join Mecanicos M2 on D.nomina2=M2.nomina where s.idSolicitud=" + id2, conn);
       ...
       public List<solicitud> GetSolicitudsById2(int id2)
       {
           ...
           using (SqlConnection conn = new SqlConnection(strConn))
           {
               ...
               SqlCommand cmd = new SqlCommand("select s.idSolicitud, s.nombreSolicitante, s.correo, s.fechaSolicitud, s.horaSolicitud, s.area, s.maquina, s.dispositivo, s.descripcionProblema, d.nomina, m.nombre, d.fechaInicio, d.horaInicio, d.diagnostico, d.tipoFalla, d.emailSent, d.nomina2, m2.nombre as nombre2, d.asignacion, r.tareasEjecutadas, r.generoParo, r.paroCorrectivo, r.paroOperativo, r.paroRefacccion, r.tiempoTotal, r.grasaUtilizada, r.refaMateHerra, c.fechaFinal, c.horaFinal, c.trabajoSanitizado, c.estatusActividad, c.firmaSolicitante, c.emailSent2 from SolicitudPreventiva as S inner join Diagnostico as D on S.idSolicitud=D.idSolicitud inner join Reparacion as R on D.idSolicitud= R.idSolicitud inner join CierreSolicitud as C on R.idSolicitud= C.idSolicitud inner join Mecanicos M on D.nomina=M.nomina inner join Mecanicos M2 on D.nomina2=M2.nomina where d.nomina= " + temp, conn);
